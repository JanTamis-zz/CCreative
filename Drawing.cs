...
namespace CCreative
{
    public static class Drawing
    {
        ...
        public static int PBO = GL.GenBuffer();
        ...
        static drawModes imgMode = drawModes.Corner;
        ...
        public static Style currentStyle = new Style(1, Color.Black, Color.White, Color.White, drawModes.Corner, drawModes.Center, angleModes.Radians);
        ...
        public static Color background(double value)
        {
        ...
        public static void imageMode(drawModes modes)
        {
            imgMode = modes;
        }
        ...
        public static void ellipse(double x, double y, double w, double h)
        {
            ...
            if (currentStyle.fillColor.A > 0 || currentStyle.strokeColor.A > 0)
            {
                ...
                double X = w * 1;//we now start at the start angle
                double Y = h * 0;
        ...
        public static void ellipse(double x, double y, double z, double w, double h)
        {
            // http://slabode.exofire.net/circle_draw.shtml
            ...
            Vector3[] vertbuffer = new Vector3[num_segments];

            Vector3 vector = new Vector3(0, 0, (float)z);
            if (CircleMode == drawModes.Center)
            {
                w /= 2;
                h /= 2;
        ...
        {
            int[] temp = new int[0];
            GL.ReadPixels(0, 0, width, height, OpenTK.Graphics.OpenGL.PixelFormat.Bgra, PixelType.UnsignedByte, temp);

            pixels = Array.ConvertAll(temp, (p => Color.FromArgb(p)));
        }

        ///-------------------------------------------------------------------------------------------------
        /// <summary>   Changes the color of any pixel. </summary>
        ///
        /// <remarks>   Jan Tamis, 27-8-2017. </remarks>
        ///
        ...
        public static void updatePixels()
        {
            if (first)
            {
                GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, width, height, 0, OpenTK.Graphics.OpenGL.PixelFormat.Bgra, PixelType.UnsignedByte, Array.ConvertAll(pixels, (p => p.ToArgb())));
                first = false;
            ...
            {
                GL.TexSubImage2D(TextureTarget.Texture2D, 0, 0, 0, width, height, OpenTK.Graphics.OpenGL.PixelFormat.Bgra, PixelType.UnsignedByte, Array.ConvertAll(pixels, (p => p.ToArgb())));
            }
        ...
        /// <param name="Image">The image to draw.</param>
        /// <param name="location">The location to draw the image to.</param>

        public static void image(PImage Image, PointF location)
        {
            image(Image, location.X, location.Y, Image.width, Image.height);
        }

        /// <param name="Image">The image to draw.</param>
        /// <param name="location">The location to draw the image to.</param>

        public static void image(PImage Image, PointF location, double width, double height)
        {
            image(Image, location.X, location.Y, width, height);
        }
        ...
        public static void textAlign(QFontAlignment Alignment)
        {
            alignment = Alignment;
        }

        static QFontAlignment alignment;

        public static void Text(PFont font, string text, double x, double y)
        {
            location.X = (float)x;
            location.Y = (float)y;
            font.qfont.ResetVBOs();
            font.qfont.PrintToVBO(text, location, currentStyle.fillColor, alignment);
            font.qfont.DrawVBOs();
            
        }

        public static void filter(filterType type, double value)
        {
            loadPixels();
            switch (type)
            {
