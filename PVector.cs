...
namespace CCreative
{
    public class PVector
    {
        ...
        {
            x = (float)(mag() * System.Math.Cos(angle));
            y = (float)(mag() * System.Math.Sin(angle));
        ...
        {
            PVector vector = new PVector(random(float.MinValue, float.MaxValue), random(float.MinValue, float.MaxValue));

            while (vector.mag() == 0)
            {
                vector.x = random(float.MinValue, float.MaxValue);
                vector.y = random(float.MinValue, float.MaxValue);
            }
        ...
        public static PVector Sub(PVector origin, PVector toSubstract)
        {
            PVector newVector = origin;
            newVector.sub(toSubstract);

            return newVector;
        }

        public static PVector Add(PVector origin, PVector toAdd)
        {
            PVector newVector = origin;
            newVector.add(toAdd);
